...
// ✅ VITE_REPO_NAME があればそれを使う（GitHub Pages移行に強い）
// 例）VITE_REPO_NAME=furocan-pwa
const DEFAULT_REPO_NAME = 'chinkasu-pwa'

export default defineConfig(({ mode }) => {
  const env = loadEnv(mode, process.cwd(), '')
  const repoName = (env.VITE_REPO_NAME || DEFAULT_REPO_NAME).trim()
      ...
      VitePWA({
        ...
        workbox: {
          ...
          navigateFallbackDenylist: [
            ...
            new RegExp(
              `^/${repoName}/.*\\.(?:js|css|png|jpg|jpeg|svg|webp|woff2|ico)$`,
            ),
          ],
